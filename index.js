...
app.post('/api/trades', async function(req, res) {
  ...
  if (!apiKey || !apiSecret) return res.status(400).json({ error: 'Нет ключей' });
  try {
    const timestamp = Date.now().toString();
    const queryString = 'category=spot&limit=50';
    const sign = crypto.createHmac('sha256', apiSecret).update(timestamp + apiKey + '5000' + queryString).digest('hex');
...
function processTrades(trades) {
  ...
  for (let i = 0; i < trades.length; i++) {
    if (!grouped[trades[i].symbol]) grouped[trades[i].symbol] = [];
    grouped[trades[i].symbol].push(trades[i]);
  }
  const result = [];
  const symbols = Object.keys(grouped);
  for (let i = 0; i < symbols.length; i++) {
    const list = grouped[symbols[i]];
    ...
    for (let j = 0; j < list.length; j++) {
      ...
      if (t.side === 'Buy') { buySum += parseFloat(t.execPrice); buyCount++; qty += parseFloat(t.execQty); }
      else { sellSum += parseFloat(t.execPrice); sellCount++; }
    }
    const avgBuy = buyCount > 0 ? buySum / buyCount : 0;
    const avgSell = sellCount > 0 ? sellSum / sellCount : 0;
    const buyAmt = avgBuy * qty;
    const pnl = (avgSell * qty) - buyAmt - fee;
    const minT = Math.min.apply(null, times);
    const maxT = Math.max.apply(null, times);
    const dur = Math.round((maxT - minT) / 60000);
    result.push({ symbol: symbols[i], entryTime: new Date(minT).toLocaleString('ru-RU'), exitTime: new Date(maxT).toLocaleString('ru-RU'), duration: dur < 60 ? dur + ' мин' : Math.round(dur/60) + ' ч', quantity: qty.toFixed(4), entryPrice: avgBuy.toFixed(4), exitPrice: avgSell.toFixed(4), amount: buyAmt.toFixed(2), commission: fee.toFixed(4), pnl: pnl.toFixed(2), pnlPercent: buyAmt > 0 ? ((pnl/buyAmt)*100).toFixed(2) : '0' });
  }
  return result;
}
